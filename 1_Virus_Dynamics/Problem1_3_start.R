...
out.TI.manual <- rbind(c(t=0, xstartTI.Manual))
...
for(k in 1:1000){
  ...
  # 
  dT <- (lambda - deltaT * Ta - beta * Ta * I) * dt
  dI <- (beta * Ta * I - deltaI * I) * dt
...
ggplot(data=out.TI.manual) +
  geom_line(aes(x=t, y=I, color="I")) +
  geom_line(aes(x=t,y=Ta, color="Ta"))
